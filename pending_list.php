...
<head>
	...
	<title>Event Mangement System: View Pending Event</title>
...
<div id="header-wrapper">
	<div id="header">
		<div id="logo">
			...
			<h4 class="topspace">View Pending Event</h4>
			</a>
		</div>
	</div>
</div>
...
<?php
	...
	$sql = "SELECT * FROM pending_event WHERE user_id=1 AND final=0";
	$res = $con->query($sql);
	if ($res->num_rows > 0) {
		while( $row = mysqli_fetch_array($res)) {
		if($row['level1'] != -1 and $row['level2'] != -1 and $row['level3'] != -1){
			echo "<a href='view_pending_event.php?id=".$row['pd_index']."'><button type='btn' class='btn btn-primary btn-lg sharp'><i class='material-icons' style='font-size:36px'>view_module	</i><br>".$row['caseName']."</br></button></a>";
		}else{
			//echo "<br></br>";
			echo "<a href='view_pending_event.php?id=".$row['pd_index']."' disabled><button type='btn' class='btn btn-primary btn-lg sharp rejected' disabled><i class='material-icons' style='font-size:36px'>cancel</i><br>".$row['caseName']."</br></button></a>";
		}
		}
	}else{
		echo "0 result";
