...
<?php
	...
	$uid = 5;
	$sql = "SELECT * FROM invitation WHERE memberid=".$uid." AND confirm = 1";
	$res = $con->query($sql);
	$member = null;
	if ($res->num_rows > 0) {
		while( $row = mysqli_fetch_array($res)) {
			...
			echo "<a href='event_dutylist.php?cid=".$row['caseID']."&uid=".$uid."'><button type='btn' class='btn btn-primary btn-lg sharp'>".strtoupper($row['caseID'])."<br>".$member."</br></button></a>";
		}
	}else{
		echo "0 result";
	}
